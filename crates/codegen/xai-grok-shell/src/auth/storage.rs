...
impl AuthFileLock {
    // TODO: delete once the token endpoint tolerates racing refreshes; this guards the rule that a refresh token can be used only once
...
/// The caller can then decide whether to skip the write (to avoid clobbering sibling scopes).
///
...
///
/// If the file exists and `read_auth_json` fails with `InvalidData`, it is renamed to `auth.json.corrupt.<millis>` in the same directory.
/// Returns the backup path. This runs before recovery writes so the original bytes are never silently lost.
pub(crate) fn backup_corrupt_auth_file(path: &Path) -> Option<PathBuf> {
    ...
    match std::fs::rename(path, &backup) {
        Ok(()) => {
            // Corrupt backups still hold token material, so keep them owner-only
...
///
/// - Missing or empty: returns an empty map (safe to write fresh)
/// - Valid JSON: returns the parsed map
/// - Non-empty corrupt JSON: backs up to `auth.json.corrupt.<millis>`, then returns an empty map so the caller can write the new credential.
///
...
///
/// The atomic path (a temp file, then a rename) needs free space of at least the file size.
/// The old file and the full temp copy coexist until the rename.
/// On a nearly-full disk that temp copy can fail with `StorageFull` (ENOSPC) even though the credentials themselves are tiny.
/// When that happens we retry with an in-place truncate-and-rewrite, which only needs the blocks the old file freed.
///
/// The in-place path is non-atomic, with two accepted trade-offs:
/// - A failed in-place write restores the prior bytes best-effort, so on-disk state ends up no worse than before the attempt.
/// - Concurrent readers can observe a torn file during the write window; the next read heals it via [`read_auth_json_or_empty_recovering_corrupt`].
///
/// Accepting the torn-read window beats persisting nothing, which would leave every concurrent process with a stale, already-revoked token.
...
/// Dispatch helper: run `atomic`, and on `StorageFull` fall back to an in-place write.
/// Split out (with `atomic` injectable) so the disk-full fallback is unit-testable without an actually-full filesystem.
...
/// Serialize `auth_store` to `path` (truncate and rewrite), owner-only (0o600) and `fsync`'d.
/// Shared core of the atomic path (which targets the temp file) and the in-place fallback (which targets `auth.json` directly).
///
/// Streams `to_writer_pretty` through a `BufWriter` so the entire JSON string is never held in memory, avoiding OOM under severe memory pressure.
fn write_store_to(path: &Path, auth_store: &AuthStore) -> std::io::Result<()> {
    ...
    // Best-effort after durable content: a chmod-only failure must not look like a failed write
    // The in-place fallback restores the prior snapshot on any `write_store_to` Err, which would discard freshly written tokens
    // The load path re-tightens on the next read
...
/// Opening with truncation first frees the old content's blocks before the new bytes are written.
/// The write therefore needs only the file size in free space, not the extra full copy the temp-file approach keeps alive.
///
/// Truncation is destructive, so the prior bytes are snapshotted first and restored best-effort if the rewrite fails partway.
/// A failed fallback must not leave an empty or torn file where a parseable (if stale) credential used to be.
/// A partial file that survives (because even the restore failed) is healed on the next read via [`read_auth_json_or_empty_recovering_corrupt`].
...
/// Inner of [`write_auth_json_in_place`] with `write` injectable so the rollback-on-failure path is unit-testable without an actually-full disk.
...
/// Best-effort rollback: rewrite `bytes` (owner-only, `fsync`'d) after a failed in-place write.
/// A torn or empty file must not replace the prior credential.
...
mod write_fallback_tests {
    ...
    /// A `StorageFull` (ENOSPC) failure on the atomic path must fall back to the in-place write so the credential still lands on disk.
    #[test]
    ...
    /// Non-ENOSPC errors must propagate unchanged and must NOT trigger the in-place fallback (e.g. a permission error should not write the file).
    #[test]
    ...
    /// A fallback write that truncates then fails must roll back to the prior bytes instead of leaving an empty/torn file.
    /// Otherwise a second disk-full failure would destroy a previously-valid credential.
    #[test]
